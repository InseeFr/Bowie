...
Une variable calculée se définit via l'onglet "Variables calculées" de n'importe quelle question du questionnaire. Il faut renseigner les paramètres suivant :

  - _Libellé_
  - _Identifiant_
  - _Formule_ : possibilité d'utiliser des if, then, else if, else
    - Exemple : somme des pourcentages du chiffre d'affaires dédiés à certaines activités ou nombre de majeurs d'un ménage
  - _Type de réponse_ : parmi Texte, Date, Nombre, Booléen (cf. Création d'une question de type réponse simple)
