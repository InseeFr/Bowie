...
1. Pour toute question créée, il faut générer les variables collectées : aller dans l'onglet `Variables collectées`, cliquer sur le bouton `Générer les variables collectées`.
    ...
    !!! tip "Nommage des variables générées"
        Il est recommandé lors de la création de vos question de garder dans un premier temps le nom des variables qui est donné par défaut lors de la génération. 
...
### Variables non collectées dans un tableau
